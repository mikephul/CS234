...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(tf.uint8, [None, img_height,img_width, nchannels * state_history])
        self.a = tf.placeholder(tf.int32, [None])
        self.r = tf.placeholder(tf.float32, [None])
        self.sp = tf.placeholder(tf.uint8, [None, img_height,img_width, nchannels * state_history])
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse):
            input_layer = tf.contrib.layers.flatten(state)
            out = layers.fully_connected(inputs=input_layer,
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        var_q = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES , scope=q_scope)
        var_target_q = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES , scope=target_q_scope)

        op = []
        for i in range(len(var_q)):
            op.append(tf.assign(var_target_q[i], var_q[i]))
    ...
    def add_loss_op(self, q, target_q):
        ...
        sample_q = self.r + self.config.gamma * tf.reduce_max(target_q, axis=1) * (1-tf.cast(self.done_mask, tf.float32))
        q_a = tf.reduce_sum(tf.one_hot(self.a, num_actions) * q, axis=1)
        self.loss = tf.reduce_mean(tf.squared_difference(sample_q, q_a))
    ...
    def add_optimizer_op(self, scope):
        ...
        var_q = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=scope)
        grad_q = adam.compute_gradients(self.loss, var_list=var_q)

        if self.config.grad_clip:
            grad_q = [(tf.clip_by_norm(grad, self.config.clip_val), var) for grad, var in grad_q if grad is not None]

        self.train_op = adam.apply_gradients(grad_q)
        grad, var = zip(*grad_q)
